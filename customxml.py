...
from pyteomics.auxiliary import FileReader, PyteomicsError, basestring, _file_obj

PY3 = sys.version_info[0] == 3

if not PY3: # Python 2.7
    from urllib2 import urlopen, URLError
else: # Python 3.x
    from urllib.request import urlopen, URLError

global rsplit_position
rsplit_position = None

def _local_name(element):
    """Strip namespace from the XML element's name"""
    global rsplit_position
    if not rsplit_position:
        try:
            tmp = element.tag.rsplit('}', 1)[-1]
            rsplit_position = len(element.tag) - len(tmp)
            return tmp
        except:
            return element.tag
    else:
        if element.tag[rsplit_position - 1] == '}':
            return element.tag[rsplit_position:]
        else:
            try:
                tmp = element.tag.rsplit('}', 1)[-1]
                rsplit_position = len(element.tag) - len(tmp)
...
class XML(FileReader):
            ...
            for k, val in types.items():
                ...
                for elem in schema_tree.iter():
                    if _local_name(elem) == 'attribute' and elem.attrib.get(
                            'type', '').split(':')[-1] in val:
                        anc = elem.getparent()
                        anc_ln = _local_name(anc)
                        while not ((anc_ln == 'complexType' and 'name' in anc.attrib) or anc_ln == 'element'):
                            anc = anc.getparent()
                            if anc is None:
                                break
                            anc_ln = _local_name(anc)
                        else:
                            if anc_ln == 'complexType':
    ...
    def _get_info(self, element, **kwargs):
        ...
        for k, v in info.iteritems():
            for t, a in converters.iteritems():
